...
class MysteryBox extends Base {
  ...
  async run({ message, args, prefix }, t) {
    ...
    if (await this.client.controllers.money.getBalance(message.author.id) < 1000) return this.respond(t('commands:mysterybox.coins'))
    ...
    const collector = message.channel.createMessageCollector(x => x, { time: 10000, max: 1, errors: ['time'] })
      .on('collect', async r => {
        if (r.content === code) {
          ...
          return this.respond(t('commands:mysterybox.code', { thing: `\`${random} sCoins!\`` }))
        } else {
          collector.stop()
          return this.respond(t('commands:mysterybox.endCode'))
        }
      })
      .on('end', async r => {
        if (collector.endReason() === 'limit') return;

        return this.respond(t('commands:mysterybox.end'))
      })
  }
}
