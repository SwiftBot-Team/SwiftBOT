            ...
            .on('collect', async (reaction, user) => {
                ...
                for (let i = 0; i < game.board.length; i++) {
                    ...
                    for (let y = 0; y < game.board.length; y++) {
                        ...
                        const button = new MessageButton()
                            .setID(`${i} - ${y}`)
                            .setStyle('gray');
                        i === Math.ceil(game.board.length / 2) - 1 ? button.setLabel(' ') : Boolean((i + 1) % 2) ? Boolean((y + 1) % 2) ? button.setEmoji(now.type) : button.setLabel(' ') : Boolean((y + 1) % 2) ? button.setLabel(' ') : button.setEmoji(now.type)
                ...
                const collector = init.createButtonCollector(button => button.clicker.user?.id === game.atual.player.id)
                    ...
                    .on('collect', async (button) => {
                        ...
                        if (game.board[x].components[y].emoji?.name !== game.atual.type && isSelect) return;
                        ...
                        if (isSelect) {
                            ...
                            for (const [key, value] of Object.entries(verify)) {
                                ...
                                if (value) {
                                    ...
                                    if (value.label !== ' ') {
                                        ...
                                        if (value.emoji?.name === game.atual.type) continue;
                                    };
                                    ...
                                    const b = new MessageButton()
                                        ...
                                        .setStyle('red');
                                    value.label === ' ' ? b.setLabel(' ') : b.setEmoji(value.emoji.name)
                            ...
                            for (let i = 0; i < game.board.length; i++) {
                                for (let j = 0; j < game.board.length; j++) {
                                    if (game.board[i].components[j].style !== 2) {
                                        ...
                                        game.board[i].components[j].label === ' ' ? b.setLabel(' ') : b.setEmoji(game.board[i].components[j].emoji.name)
                            ...
                            if (selected.emoji?.name === game.atual.type) {
