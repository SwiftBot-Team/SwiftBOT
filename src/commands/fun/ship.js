...
class Ship extends Command {
  ...
  async run({ message, args, prefix }, t) {
    ...
    if (!args[0]) return message.respond(t('commands:ship.noArgs'));
    ...
    if (!user1) return message.respond(t('commands:ship.noUserFound', { user: args[0] }));
    if (!user2) return message.respond(t('commands:ship.noUserFound', { user: args[1] ? args[1] : args[0] }));
    ...
    const img = await new ShipImg().loadShip(user1, user2, chance)

    message.respond(`${chance}% - ${probabilidades[porcentagem]}`)
