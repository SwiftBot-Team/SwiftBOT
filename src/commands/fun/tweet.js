...
class Tweet extends Command {
    ...
    async run({ message, args, prefix }, t) {
        if (!args[0]) return message.respond(t('commands:tweet.noArgs1', { member: message.author.id }));

        if (!args[1]) return message.respond(t('commands:tweet.noArgs2', { member: message.author.id }));
