...
class Ressortear extends Base {
    ...
    async run({ message, args, prefix }, t) {
        ...
        if (!args[0]) return message.respond(t('commands:ressortear.noArgs', { member: message.author.id }));
        ...
        }).then().catch(err => message.respond(t('commands:ressortear.noGiveawayFound', { member: message.author.id })))
