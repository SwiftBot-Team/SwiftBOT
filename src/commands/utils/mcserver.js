...
class McServer extends Base {
  ...
  async run({ message, args, prefix }, t) {
    ...
    if (!args[0]) return message.respond(t('commands:mcserver.error'));

    const [mcIP, port] = args;
    try {
      const { data } = await get(`https://api.minetools.eu/ping/${mcIP}${port ? `/${port}` : ''}`)

      if (!data) return message.respond(t('commands:mcserver.reqError'));
      ...
      const send = await message.respond(status, true, { image: data.latency ? `http://status.mclive.eu/${mcIP}/${mcIP}/25565/banner.png` : ``, author });
      ...
      console.log(error);
      return message.respond(t('commands:mcserver.reqError'))
    }
  }
}
