...
class Docs extends Base {
  ...
  async run({ message, args, prefix, language }, t) {
    ...
    if (!search) return message.respond(t('commands:docs.error'))

    const { data } = await axios.get(`https://djsdocs.sorta.moe/v1/main/master/embed?q=${search}`)
    if (!data || data === undefined || !data.fields || !data.fields.length) return message.respond(t('commands:docs.nothing', { item: search }))
