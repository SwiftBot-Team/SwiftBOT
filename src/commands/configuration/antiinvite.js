...
class Antiinvite extends Base {
    ...
    async run({ message, args, prefix }, t) {
        ...
        if (!args[0]) return this.respond(t('commands:antiinvite.noArgs', { member: message.author.id, prefix }));
        ...
        if (['disable', 'desabilitar', 'desativar', 'off', 'turn-off'].includes(args[0].toLowerCase())) {
            ...
            if (!ref.val() || !ref.val().stats) return this.respond(t('commands:antiinvite.isDisabled', { member: message.author.id }));

            this.client.database.ref(`SwiftBOT/config/antiinvite/${message.guild.id}/stats`).set(false);

            return this.respond(t('commands:antiinvite.disabled', { member: message.author.id }))
        }

        if (['enable', 'on', 'turn-on', 'ligar', 'ativar'].includes(args[0].toLowerCase())) {

            if (ref.val() && ref.val().stats) return this.respond(t('commands:antiinvite.isEnabled', { member: message.author.id }));

            this.client.database.ref(`SwiftBOT/config/antiinvite/${message.guild.id}/stats`).set(true);

            return this.respond(t('commands:antiinvite.enabled', { member: message.author.id }));
        ...
        if (['addperm', 'add', 'addpermissão', 'adicionar', 'adicionarpermissão'].includes(args[0].toLowerCase())) {
            ...
            if (!ref.val()) return this.respond(t('commands:antiinvite.noStarted', { member: message.author.id }));

            if (!args[1]) return this.respond(t('commands:antiinvite.noArgs1', { member: message.author.id }));
            ...
            if (!perm) return this.respond(t('commands:antiinvite.noPermFound', { member: message.author.id }));

            this.respond(t('commands:antiinvite.added', { member: message.author.id }));
        ...
        if (['removeperm', 'remove', 'removerpermissão', 'remover', 'removerpermissão'].includes(args[0].toLowerCase())) {
            ...
            if (!ref.val().permissions || !Object.values(ref.val().permissions).find(r => r.id === perm.id)) return this.respond(t('commands:antiinvite.noInsertRole', { member: message.author.id }))

            this.respond(t('commands:antiinvite.removed', { member: message.author.id }));
