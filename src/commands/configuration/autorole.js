...
class AutoRole extends Base {
  ...
  async run({ message, args, prefix }, t) {
    if (!args[0]) return message.channel.send(new this.client.embed().setDescription('<:errado:739176302317273089> » ' + t('commands:autorole.use', { prefix })));
    ...
    const role = message.guild.roles.cache.get(args[1]) || message.guild.roles.cache.find(r => r.name === args[1]) || message.mentions.roles.first();
      ...
      if(['add', 'adicionar'].includes(args[0].toLowerCase())) {
        if(!args[1]) return message.channel.send(new this.client.embed(message.author).setDescription(t('commands:autorole.noArgs1', {member: message.author.id, acao: 'adicionar'})))
        const roles = [];
        ...
        if (ref.val() && ref.val().includes(role.id)) return message.channel.send(new this.client.embed(message.author).setDescription(t('commands:autorole.exists', { member: message.author.id, role: role.id })))
