...
class Unlockcmd extends Base {
    ...
    async run({ message, args, prefix }, t) {
        ...
        if (!toLock) return message.respond(t('commands:unlockcmd.noArgs', { member: message.author.id }));
        ...
        if (!cmd[0] && !['*', 'all', 'todos'].includes(toLock.toLowerCase())) return message.respond(t('commands:unlockcmd.noFound', { member: message.author.id }));

        if (['*', 'all', 'todos'].includes(toLock.toLowerCase())) {

            this.client.database.ref(`SwiftBOT/Servidores/${message.guild.id}/lockedcmds`).remove();

            return message.respond(t('commands:unlockcmd.allUnlocked'));
        }
        ...
        if (categories.includes(toLock.toLowerCase())) {
            ...
            return message.respond(t('commands:unlockcmd.categoryUnlocked', { categoria: toLock }));
        ...
        return message.respond(t('commands:unlockcmd.oneUnlocked', { comando: toLock }));
    }
}
