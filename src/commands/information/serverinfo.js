...
class ServerInfo extends Base {
  ...
  run({ message, args, prefix }, t) {
    ...
    function checkDays(date) {
      ...
      return days + (days == 1 ? ` ${t('utils:day')}` : ` ${t('utils:days')}`);
    };
    ...
    if (args[0]) {
      ...
      const roles = 
        guild.roles.cache.map(g => '`'+g.name+'`').join(',').length >= 1024 ?
          t('commands:serverinfo.toMuch')
          : guild.roles.cache.map(g => '`'+g.name+'`').join(',')
      ...
      Embed
        ...
        .addField('<:badgeA:795773919465963551> **»** ' + t('commands:serverinfo.roles'), '▫️ ' + `${roles}`)
