...
class ServerIcon extends Base {
  ...
  run({ message, args, prefix }, t) {
    ...
    if (args[0]) {
      ...
      if (!guild) return message.respond(t('commands:servericon.notFound'))

      Embed
        .setAuthor(guild.name, guild.iconURL())
        .setDescription(t('commands:servericon.description', { link: guild.iconURL({ size: 4096, dynamic: true }) }))
